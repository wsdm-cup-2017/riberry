...
public class FeatureExtractor {
	...
	public static String extractedrecord;
	...
	public FeatureExtractor(MwRevision revision){
		int result = 0;
...
//			logConfiguration(config);
	...
	public static void executePipeline(MwRevision revision){
		//ItemStore itemStore = new SQLItemStore();
		ItemStore itemStore = new MemoryItemStore();
		
		List<Feature> features = FeatureList.getFeatures(ALL_FEATURES_ENABLED);

		File featureFile = new File("tmp");
		RevisionProcessor nextProcessor = new CsvFeatureWriter(featureFile, features);
		RevisionProcessor nextProcessor_copy=nextProcessor;
...
//		teeProcessor.add(samplingSHA1Processor);
		...
		nextProcessor = new ActionStatisticsProcessor(teeProcessor, featureFile.getAbsoluteFile() + "_monthlyActionDistribution.csv");
...
//		dumpFileProcessor.processDumpFileContents(dumpFile.getDumpFileStream(), dumpFile);
		...
		dumpFileProcessor.processDumpFileContents(revision);
		extractedrecord=((CsvFeatureWriter)nextProcessor_copy).extractedrecord;
	}
