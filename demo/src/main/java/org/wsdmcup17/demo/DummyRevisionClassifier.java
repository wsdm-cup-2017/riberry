...
/**
 * A Wikidata Toolkit-based revision processor that classifies all revisions as
 * non-vandalism and hence sends a score 0f 0.0 to the server for each revision.
 */
public class DummyRevisionClassifier implements MwRevisionProcessor {

	private static final Logger LOG = LoggerFactory.getLogger(DummyRevisionClassifier.class);

	private static final String LOG_MSG_STARTING = "Starting...", LOG_MSG_CURRENT_STATUS = "Current status:",
			LOG_MSG_FINAL_RESULT = "Final result:", LOG_MSG_NUM_REVISIONS = "   Number of revisions: %s",
			LOG_MSG_NUM_REVISIONS_REGISTERED = "   Number of registered revisions: %s";

	private static final int LOG_STATISTICS_INTERVAL = 10000;
	...
	private float classifyRevision(MwRevision revision, CSVRecord metadata) {
		...
		try {
			...
			Booster booster = XGBoost.loadModel("./model/xgb.model");
			DMatrix testMat = new DMatrix("./model/dtest.buffer");
	...
	private void sendClassificationResult(long revisionId, float classificationScore) {
		try {
	...
	private void printStatistics() {
		...
		LOG.info(String.format(LOG_MSG_NUM_REVISIONS_REGISTERED, numRevisionsFromRegisteredUsers));
	}
}
