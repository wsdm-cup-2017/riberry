...
public class DTreeCateg2 implements Serializable{
	...
	private static final int INDEX_SKIP=10000;//3
	...
	private static final int MIN_NODE_SIZE=1000;//5
...
//		System.out.println("bootstrap N:"+N+" size of bootstrap:"+bootstrap.size());
	...
	private TreeNode CreateTree(ArrayList<ArrayList<String>> train, int ntree){
		...
		boolean[] restfeature =new boolean[RandomForestCateg.M];
		for(int i=0;i<RandomForestCateg.M;i++){
			restfeature[i]=true;
		}
		RecursiveSplit(root,ntree,restfeature);
		return root;
	}
	/**
	 * 
...
//				copy.right=right.clone();
	 ...
	 * 
	 */
	public ArrayList<String> CalculateClasses(ArrayList<ArrayList<String>> testdata, int treenumber){
		ArrayList<String> predicts = new ArrayList<String>();
		for(ArrayList<String> record:testdata){
			String Clas = Evaluate(record);
			if(Clas==null)
	...
	public ArrayList<String> CalculateClasses_NoClass(ArrayList<ArrayList<String>> testdata, int treenumber){
		ArrayList<String> predicts = new ArrayList<String>();
		for(ArrayList<String> record:testdata){
			String Clas = Evaluate_NoClass(record);
			if(Clas==null)
				System.out.println("Evaluation Nul hua");
			predicts.add(Clas);
		}
			...
			while (true) {				
				if(evalNode.isLeaf)
					...
					// if its categorical
						...
						double Compare;
						double Actual;
						if(evalNode.splitValue.equals("")||record.get(evalNode.splitAttributeM).equals("")){
							//System.out.println("Error: Double.parseDouble: java.lang.NumberFormatException: multiple points: "+record.get(evalNode.splitAttributeM));
							Compare = 0;
							Actual = 1;
						}else if(evalNode.splitValue.split(".").length>2||  record.get(evalNode.splitAttributeM).split(".").length>2){
							//System.out.println("Error: Double.parseDouble: java.lang.NumberFormatException: multiple points: "+record.get(evalNode.splitAttributeM));
							Compare = 0;
							Actual = 1;
						}else if(isNumeric(record.get(evalNode.splitAttributeM))==false){
							//System.out.println("Error: Double.parseDouble: java.lang.NumberFormatException: multiple points: "+record.get(evalNode.splitAttributeM));
							Compare = 0;
							Actual = 1;
						}else{
							Compare = Double.parseDouble(evalNode.splitValue);
							Actual = Double.parseDouble(record.get(evalNode.splitAttributeM));
						}
						//////
...
//							System.out.println("going in child :"+evalNode.label);
	 ...
	 * 
	 */
	public String Evaluate_NoClass(ArrayList<String> record){
		TreeNode evalNode=root;		
			while (true) {				
				if(evalNode.isLeaf)
					return evalNode.Class;
					...
					// if its categorical
						...
						double Compare;
						double Actual;
						//System.out.println("size "+record.size()+" a " + evalNode.splitAttributeM);
						if(evalNode.splitValue.equals("")||record.get(evalNode.splitAttributeM).equals("")){
							//System.out.println("Error: Double.parseDouble: java.lang.NumberFormatException: multiple points: "+record.get(evalNode.splitAttributeM));
							Compare = 0;
							Actual = 1;
...
//							System.out.println("going in child :"+evalNode.label);
	 ...
	 */
	private void RecursiveSplit(TreeNode parent, int Ntreenum, boolean[] restfeature){
		String tmps2="";
		for(int k=0;k<restfeature.length;k++){
			tmps2=tmps2+" "+restfeature[k];
		}
		System.out.println("SSSSSSSSSSSSSSSS "+tmps2);
		
		
		boolean[] tmprestfeature=restfeature.clone();
		...
		if (!parent.isLeaf){
			...
			//System.out.println("Step B: "+parent.ChildNode.size());
			for(TreeNode TN: parent.ChildNode){
				TN = new TreeNode();
				TN.generation = parent.generation+1;
			}
			
			ArrayList<Integer> vars=GetVarsToInclude(tmprestfeature);//randomly selects Ms.Nos of attributes from M
//			for(int k:vars){
...
//			}
			...
			//-------------------------------Step C
			//System.out.println("Step C: "+vars.size());
			int tmpcount=0;
			for (int m:vars){//m is from 0-M
				tmpcount++;
				//System.out.println("In Step C: tmpcount= "+tmpcount+" / "+vars.size());
				
				SortAtAttribute(parent.data,m);//sorts on a particular column in the row
				//System.out.println("In Step C: tmpcount= "+tmpcount+" / "+vars.size()+" a");
				...
				//System.out.println("In Step C: tmpcount= "+tmpcount+" / "+vars.size()+" b");
				...
				//System.out.println("In Step C: tmpcount= "+tmpcount+" / "+vars.size()+" c");
				if (DataPointToCheck.size() > MIN_SIZE_TO_CHECK_EACH){
					if(DataPointToCheck.size()<INDEX_SKIP){
						for (int i=0;i<DataPointToCheck.size();i+=100){
							System.out.println("In Step C: tmpcount= "+tmpcount+" / "+vars.size()+" c i= "+i+" / "+DataPointToCheck.size());
					...
					for (int k:DataPointToCheck){
						tmpc++;
						//System.out.println("In Step C: tmpcount= "+tmpcount+" / "+vars.size()+" c tmpc= "+tmpc);
						CheckPosition(m,k, Nsub, lowestE, parent, Ntreenum);
						if (lowestE.d == 0)//lowestE now has the minimum conditional entropy so IG is max there
							break;
					}
				}
				//System.out.println("In Step C: tmpcount= "+tmpcount+" / "+vars.size()+" d");
				if (lowestE.d == 0)
					break;
			}
			//System.out.println("Adding "+parent.ChildNode.size()+" children at level: "+parent.generation);
			//-------------------------------Step D
			//System.out.println("Step D:" + parent.ChildNode.size());
			int count=0;
			for(TreeNode Child:parent.ChildNode){
				...
				if(!Child.isLeaf){
					tmprestfeature[parent.splitAttributeM]=false;
					String tmps="";
					for(int k=0;k<tmprestfeature.length;k++){
						tmps=tmps+" "+tmprestfeature[k];
					}
					System.out.println("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX "+tmps +" nodeid= "+ parent.nodeid);
					
					boolean flag=true;
					for(int i=0;i<tmprestfeature.length;i++){
						if(tmprestfeature[i]){
							flag=false;
						}
					}
					if(flag){
						...
						RecursiveSplit(Child, Ntreenum,tmprestfeature);
					}
				}
				//System.out.println("In Step D: count: " + count);
			}
	...
	private class AttributeComparatorReal implements Comparator<ArrayList<String>>{		
		...
		public int compare(ArrayList<String> arg1, ArrayList<String> arg2) {//compare value of strings
			...
			//////tuoyu
//			if(arg1.get(m)==null || arg2.get(m)==null){
//				System.out.println("Error: Double.parseDouble: java.lang.NumberFormatException: multiple points: null");
//				return 0;
//			}
			double a2;
			double b2;
			if(arg1.get(m).equals("NA")|| arg1.get(m).split(".").length>2  || arg1.get(m).equals("") ||isNumeric(arg1.get(m))==false ||arg1.get(m).equals(".") ){//11/19
				a2=-1;
			}else{
				a2 = Double.parseDouble(arg1.get(m));
			}
			
			if(arg2.get(m).equals("NA")|| arg2.get(m).split(".").length>2  || arg2.get(m).equals("") ||isNumeric(arg2.get(m))==false || arg2.get(m).equals(".")){//11/19
				b2=-1;
			}else{
				b2 = Double.parseDouble(arg2.get(m));
			}
	 ...
	 */
	private ArrayList<Integer> GetVarsToInclude(boolean[] tmprestfeature) {
		
		boolean[] whichVarsToInclude=new boolean[RandomForestCateg.M];
		for(int i=0;i<RandomForestCateg.M;i++){
			whichVarsToInclude[i]=false;
		}
		int[] tmp=new int[RandomForestCateg.M];
		...
		if(count<=RandomForestCateg.Ms){
			for (int i=0;i<RandomForestCateg.M;i++){
				if(tmp[i]==0){
					whichVarsToInclude[i]=true;
				}
			}
		}else{
			while (true){
				...
				for(int k=0;k<RandomForestCateg.M;k++){
					if(tmp[k]==0){
						co++;
					}
					if(co==a){
